#!/bin/sh
# Functional tests for packaging/scripts/install.sh.
#
...
BINARY="$REPO_ROOT/core/rust/target/release/hara"
...
if [ ! -x "$BINARY" ]; then
  ...
  echo "run: cargo build --release --manifest-path core/rust/Cargo.toml --bin hara" >&2
  exit 1
fi
