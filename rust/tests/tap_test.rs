...
fn publisher_signature_requires_an_authorized_non_revoked_key() {
    ...
    keys.insert("publisher-1".into(), PublisherKey {
        public_key: hex(&signing.verifying_key().to_bytes()),
        coordinates: vec!["acme:widgets/core".into()],
        revoked: false,
    });
    let policy = IdentityPolicy { revision: "identity-commit".into(), publisher_keys: keys };
    tap::authorize(&policy, "publisher-1", "acme:widgets/core", intent, &signature).unwrap();
    assert!(tap::authorize(&policy, "publisher-1", "other:widgets/core", intent, &signature).is_err());
    assert!(tap::authorize(&policy, "publisher-1", "acme:widgets/core", b"changed", &signature).is_err());
}
...
fn signed_identity_policy_is_verified_against_the_pinned_tap_root() {
    ...
    fs::write(root.join("identity.edn.sig"), hex(&signing.sign(policy.as_bytes()).to_bytes())).unwrap();
    for arguments in [["init"].as_slice(), ["add", "."].as_slice(), ["-c", "user.name=test", "-c", "user.email=test@example.test", "commit", "-m", "policy"].as_slice()] {
        assert!(Command::new("git").arg("-C").arg(&root).args(arguments).status().unwrap().success());
    }
...
fn signed_initializer_creates_a_pair_without_private_key_material() {
    ...
    let policy = format!("{{:identity/format 1 :identity/root-key \"{}\" :publisher-keys {{}}}}\n", hex(&root));
    let initialized = tap::initialize_signed("acme", &registry, &identity, &root, &policy, &hex(&signing.sign(policy.as_bytes()).to_bytes())).unwrap();
    ...
    assert!(registry.join(".github/workflows/verify-request.yml").is_file());
    assert!(!fs::read_to_string(identity.join("identity.edn")).unwrap().contains("private"));
