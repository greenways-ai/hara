...
fn application_request_source(project: &str, cwd: &str, options: &Options) -> String {
    let mut capabilities = vec![Form::Keyword("file".into())];
    if options.allow_process {
...
fn usage() {
    ...
    println!();
    println!("Usage:");
    println!("  hara-app [--project PATH] [--allow-process] [--allow-net] -- [ARGS...]");
    println!();
    ...
    fn application_arguments_are_separate_from_runner_options() {
        assert_eq!(
            ...
            Command::Run(Options {
                ...
                allow_process: true,
                native_sockets: false,
                arguments: strings(&["vault", "init", "--vault", "/tmp/demo.git"]),
            })
        );
    }

    #[test]
    fn application_request_is_parseable_for_arbitrary_argument_text() {
        let options = Options {
            project: PathBuf::from("."),
            allow_process: true,
            native_sockets: true,
            arguments: strings(&["quoted\"value", "line\nbreak", "--help"]),
        };
        let source = application_request_source("demo", "/tmp/demo", &options);
        let forms = parse_forms(&source).unwrap();
        assert_eq!(forms.len(), 3);
        assert!(source.contains(":hara.cli/arguments"));
        assert!(source.contains(":process"));
        assert!(source.contains(":network"));
    }

    #[test]
    fn application_result_protocol_decodes_raw_process_output() {
        assert_eq!(
            decode_application_result(
                "{:hara.app/stdout \"ok\\n\" :hara.app/stderr \"\" :hara.app/exit 0}"
            )
            .unwrap(),
        ...
        assert_eq!(decode_application_result("{:ok true}").unwrap(), None);
    }
}
