...
//!   `hara-specs/01-lang/020-data-structures/draft/conformance/hash-parity.edn`.)
...
mod tests {
    ...
    fn corpus_path(relative: &str) -> Option<std::path::PathBuf> {
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .ancestors()
            .map(|root| root.join("hara-specs").join(relative))
    ...
    /// normative corpus hara-specs/01-lang/020-data-structures/draft/conformance/
